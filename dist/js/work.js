...
var FadeInTargetModule = function () {
  function init() {
...
var WorkModule = function () {
  ...
  function init() {
    ...
    var $tilters = document.querySelectorAll('.tilter');
    var $closeBtn = document.querySelector('#close-btn');
    var $workOverlay = document.querySelector('#workOverlay');
    var $workInfoHeading = document.querySelector('.work-info-heading');
    var $workInfoDescription = document.querySelector('.work-info-description');
    var $workInfoTeam = document.querySelector('.work-info-team');
    var $workInfoLink = document.querySelector('.work-info-link');
    var $workInfoText = document.querySelector('.work-info-text').children;
    ...
    function openWorkItem(e) {
      ...
      var workEnterTl = new TimelineMax();
      workEnterTl.to('.row-1', 0.5, { opacity: 0.25, ease: Circ.easeOut }, 'fade').to('.row-2', 0.5, { opacity: 0.25, ease: Circ.easeOut }, 'fade').to($workOverlay, 0.5, { y: '0%', zIndex: 3, display: 'block', ease: Circ.easeInOut }, 'fade+=0.5').staggerFromTo('.x-line', 0.25, { drawSVG: 0 }, { drawSVG: "100%", ease: Circ.easeOut }, 0.08, 'enter').staggerFrom($workInfoText, 0.5, { yPercent: 100, autoAlpha: 0, force3D: true, ease: Circ.easeOut }, .08, 'enter');
    }
    function closeWorkItem(e) {
      e.preventDefault();
      var workLeaveTl = new TimelineMax();
      workLeaveTl.staggerTo('.x-line', 0.25, { drawSVG: 0, ease: Circ.easeOut }, 0.08).to($workOverlay, 0.5, { y: '100%', zIndex: 0, display: 'none', ease: Circ.easeInOut }, 'fade').to('.row-2', 0.5, { opacity: 1, ease: Circ.easeOut }, 'fade').to('.row-1', 0.5, { opacity: 1, ease: Circ.easeOut }, 'fade');
    }
    $tilters.forEach(function (tilter) {
      return tilter.addEventListener('click', openWorkItem);
    });
    $closeBtn.addEventListener('click', closeWorkItem);
    $closeBtn.addEventListener('mouseenter', function (e) {
      e.preventDefault();
      var xhoverETl = new TimelineMax();
      xhoverETl.staggerFrom('.x-line', 1, { stroke: '#ec007c', ease: new SlowMo(0.2, 0.2) }, 0.08, 'enter').staggerTo('.x-line-1', 0.25, { drawSVG: 0, ease: Circ.easeOut }, 0.08, 'enter');
    });
    $closeBtn.addEventListener('mouseleave', function (e) {
      e.preventDefault();
      var xhoverLTl = new TimelineMax();
      xhoverLTl.staggerFromTo('.x-line', 1, { stroke: '#4bfffc' }, { stroke: '#3cfc81', ease: new SlowMo(0.2, 0.2) }, 0.08, 'leave').staggerTo('.x-line-1', 0.25, { drawSVG: "100%", ease: Circ.easeOut }, 0.08, 'leave');
    });
  }
